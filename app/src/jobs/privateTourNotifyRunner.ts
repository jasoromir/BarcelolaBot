...
export function createPrivateTourNotifyRunner(deps: PrivateTourNotifyRunnerDeps): PrivateTourNotifyRunner {
  ...
  // Re-entrancy guard: an event only gets marked handled (notifyStore.record)
  // AFTER its sends complete, and each send now goes through the bot's single
  // global send queue with a humanized typing delay — easily longer than the
  // 60s poll interval. Without this guard, a slow tick is still in flight
  // when the next one starts, both see "not yet handled", and both send —
  // seen live 2026-07-22 as a duplicated private-tour reminder to guide אדיר.
  // Mirrors the same fix already applied to reminders/runner.ts.
